...
fallback), which drives the 384×128 HUB75 wall with a zero-CPU PIO+DMA engine.
...
The link carries *only frame content* (a 192 KB packed bit-plane buffer — identical
...
| `--pio-clkdiv` | 3 | RP1-PIO clock divisor; per-lane rate ≈ 200 MHz/(2·div), so the 192 KB v2 frame floor ≈ 12 ms at div 3 (≈83 fps ceiling — above the 60 target, and the render∥send overlap hides it), 8 ms at div 2. **Lower = faster, more SI risk.** |
...
| `--width` / `--height` | 384 / 128 | Render size (defaults to the full wall from `config.py`). Only touch for experiments. |
...
|---|---|---|
| `PANEL_COLS` / `PANEL_ROWS` | 6 / 4 | **The geometry knobs** — the physical panel grid (24 tiles = 384×128). Everything else derives. Set `PANEL_ROWS=2` for the staged 6×2 bring-up. `PANEL_ROWS` must divide by `PARALLEL_CHAINS`. |
| `ROWS`,`COLS` | 32,64 | One panel's pixels. |
| `PARALLEL_CHAINS` | 2 | HUB75 chains — **fixed by the HAT + the u16 engine** (12 RGB bits on GP0–11). Not a knob. |
| *derived:* `CHAIN` / `CHAIN_WIDTH` | 12 / 768 | Panels **per chain** and one chain's electrical strip width. `CHAIN` **must equal firmware `PANELS_IN_CHAIN`**; `CHAIN_WIDTH` is the firmware's `W`. A chain spanning 2 panel rows serpentines, so these are *not* the wall's width — see `hub75.to_chains`. |
| `CHAIN_ORDER` / `ROW_ROTATE_180` | derived | Which panel sits where in each chain's strip, + which panel rows are mounted 180°-rotated. Generated by `config.serpentine()`; override by hand if you cable something asymmetric. Confirm on the wall with `rayglow.spi_test` (per-panel ID dots). |
...
| `SINGLE_CHAIN` | False | Single-chain fallback rig (Adafruit-HAT era) — drives every panel on ONE chain via the u8 engine. Leave False on the two-chain HAT. |
...
|---|---|---|
| `OE_GAIN` | 192 | **The brightness knob.** Scales the BCM output-enable (lit) intervals; brightness climbs ~linearly until the top plane's on-time approaches the shift time, then it starts eating refresh. 192 on the 768-wide wall = the same 72% duty (so same brightness *and* average LED current) the 256-wide wall had at 64 — see the width/brightness note below. |
| `DATA_CLK_DIV` | `(3,0)` = 25 MHz | **HUB75 pixel clock** = sysclk / (2·div). Faster = brighter + higher refresh, more SI risk down the chain. `(2,0)` = 37.5 MHz was validated clean — but only over a **4**-panel chain; the v2 wall runs 12 deep. If it glitches, *raise* the divisor: `(4,0)` = 18.8 MHz still refreshes ~153 Hz at gain 128. |
| `PANELS_IN_CHAIN` | 12 | Panels per chain; `W = 64 ×` this. **Must equal the Pi's `config.CHAIN`** (below). Also sets RAM: 32 KB per panel-per-chain double-buffered; 15 is the linkable ceiling (16 overflows 512 KB by 96 B). |
...
### Chain width vs brightness vs refresh (why `OE_GAIN` moved 64 → 192)

Per bit-plane the row SM waits on **both** the data shift (`irq 4`) and the OE lit
interval (`irq 7`), and they overlap — so a plane costs `max(shift, lit)`. Widening the
chain lengthens `shift` only, which *dilutes duty cycle*: the panel spends more of each
plane dark. Tripling 256 → 768 px drops duty 72% → 39% at a fixed gain (visibly dimmer).
Raising `OE_GAIN` buys it back by lengthening `lit`, paid for in refresh:

| Wall | `PANELS_IN_CHAIN` | `OE_GAIN` | Refresh | Duty (∝ brightness) |
|---|---|---|---|---|
| v1 256×64 | 4 | 64 | ~430 Hz | 72% |
| v2 384×128 | 12 | 64 | ~232 Hz | 39% — too dim |
| **v2 384×128** | **12** | **192** | **~143 Hz** | **72% — v1 parity** |

143 Hz is well above flicker perception (it shows up in phone video sooner than to the
eye). Same duty = same average LED current, so no thermal change. Colour is unaffected —
gain scales all planes, so the binary weighting (and thus ratios) holds.

## Geometry changes: the Pi and firmware must move together

Both sides derive the frame byte-count from the geometry, and the RX DMA waits for
**exactly** that many bytes — a mismatch desyncs the link **silently**. So a geometry
change is always a *pair* of changes. `uv run --with numpy tools/fold_check.py` prints
the `PANELS_IN_CHAIN` the Pi currently expects.

| Rig | Firmware (reflash) | Pi (`config.py`, live) | Frame |
|---|---|---|---|
| **v2 wall 6×4, two chains (production)** | `--features two-chain`, `PANELS_IN_CHAIN=12` | `PANEL_COLS=6`, `PANEL_ROWS=4` | 192 KB |
| v2 staged 6×2 (chain-length SI test) | `--features two-chain`, `PANELS_IN_CHAIN=6` | `PANEL_COLS=6`, `PANEL_ROWS=2` | 96 KB |
| v1 wall 4×2, two chains (retired) | `--features two-chain`, `PANELS_IN_CHAIN=4` | `PANEL_COLS=4`, `PANEL_ROWS=2` | 64 KB |
| Single chain, u8 engine (Adafruit-HAT era) | default features, `PANELS_IN_CHAIN=` panel count | `SINGLE_CHAIN=True` | ½ the above |

The Pi side needs no reflash — the packer sizes the payload from the frame it's handed
(`pack()` infers width; `to_chains` does the serpentine fold), and the PIO shim sizes its
DMA buffer from the first payload. The firmware side is compile-time (`W`/`H` are const
generics), hence the reflash.
