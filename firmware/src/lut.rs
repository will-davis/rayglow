//! Gamma / CIE-style brightness correction lookup tables.
//!
//! Ported verbatim from kjagiello's `hub75-pio-rs` (`src/lut.rs`) — it is
//! HAL-agnostic (only `embedded-graphics` + `libm`), so nothing needed changing
//! for the RP2350. A `GammaLut<B, C, Init>` maps an `embedded-graphics`
//! `RgbColor`'s 0..=MAX component values to gamma-corrected `B`-bit plane
//! values, preserving the color quality of the original pipeline.
